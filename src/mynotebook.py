...
def show_it(orf, data):
    ...
    # Create histogram
    counts, bins, patches = plt.hist(scores, bins=30, edgecolor='black', range=numrange, align='mid', alpha=0.7)
    ...
    if x_vals is not None and density is not None:
        ...
        scaled_density = scale_kde_to_histogram(density, x_vals, np.max(counts), bin_width)
...
def show_two(orf, data_good, data_bad):
    ...
    counts_good, bins_good, _ = ax1.hist(scores_good, bins=30, alpha=0.5, label='Intact', 
                                          edgecolor='black', color='black', range=numrange, align='mid')
    ...
    if x_vals_good is not None and density_good is not None:
        ...
        scaled_density_good = scale_kde_to_histogram(density_good, x_vals_good, np.max(counts_good), bin_width)
        ax1.plot(x_vals_good, scaled_density_good, 'darkblue', linewidth=2, 
                label='Intact KDE', alpha=0.8, linestyle='--')

    ax2.set_ylabel('Defective count')
    counts_bad, bins_bad, _ = ax2.hist(scores_bad, bins=30, alpha=0.5, label='Defective', 
                                        edgecolor='black', color='red', range=numrange, align='mid')
    ...
    if x_vals_bad is not None and density_bad is not None:
        ...
        scaled_density_bad = scale_kde_to_histogram(density_bad, x_vals_bad, np.max(counts_bad), bin_width)
        ax2.plot(x_vals_bad, scaled_density_bad, 'darkred', linewidth=2, 
                label='Defective KDE', alpha=0.8, linestyle='--')
